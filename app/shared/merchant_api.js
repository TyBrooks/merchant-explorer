...
app.service('merchantApi', ["$http", "config", "$q", function($http, config, $q) {
  ...
  this.searchApiCall = function( apiSearchParams ) {
    ...
    currentPromise = $http.get(searchUrl, { params: apiSearchParams, withCredentials: true } );
  ...
  this.retrieveApiCall = function( apiRetrieveParams ) {
    ...
    currentPromise = $http.get(retrieveUrl, { params: apiRetrieveParams, withCredentials: true } );
    return currentPromise;
  }
  ...
  this._getMockMerchantData = function(ids) {
    ...
    ids.forEach(function(id) {
      toReturn.push({
        ...
        overallRestricted: affiliatable(),
